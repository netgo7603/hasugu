...
def update_blog_index(posts: list):
    ...
    for p in posts:
        blog_id = p.get("log_no")
        slug = POSTS_METADATA[blog_id]["slug"] if blog_id in POSTS_METADATA else p.get("slug", "")
        cover = get_best_cover_image(blog_id, []) if blog_id else p.get("cover_image", "")
